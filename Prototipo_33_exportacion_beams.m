...
% Recolector de curvas para exportar
...
for i = 1:N
    ...
    if i == ply_gap
        ...
        % ====== SUBMUESTREO (solución A): limitar Bézier a 30 puntos ======
        M = 20;                                  % nº máximo de puntos deseados
        M = max(2, min(M, numel(xbL)));          % seguridad
        idxL = round(linspace(1, numel(xbL), M));
        idxR = round(linspace(1, numel(xbR), M));
        xbL  = xbL(idxL);  ybL = ybL(idxL);
        xbR  = xbR(idxR);  ybR = ybR(idxR);
        % ==================================================================

        plot(xbL, ybL, 'k-', 'LineWidth', lw);
        plot(xbR, ybR, 'k-', 'LineWidth', lw);

        % Añadir las dos Bézier a exportar (ya submuestreadas)
        curves{end+1} = struct('x', xbL, 'y', ybL); %#ok<SAGROW>
        curves{end+1} = struct('x', xbR, 'y', ybR); %#ok<SAGROW>

        % (opcional) ver k's
        fprintf('kL=%.4f, kR=%.4f -> k_common=%.4f (M=%d)\n', kL, kR, k_common, M);
...
%% ====== EXPORTACIÓN: GRID + CBEAM (visible y válido para flexión) ======
beam.E   = 7.0e4;  beam.G = 2.6e4;  beam.NU = 0.3;  beam.RHO = 0.0;
beam.b   = 0.10;   beam.h = 0.10;
fname_bdf_beam = 'curvas_2D_CBEAM.bdf';
write_bdf_grids_and_cbeams(fname_bdf_beam, curves, 1, beam);
fprintf('> Exportado nodos + CBEAM a %s\n', fname_bdf_beam);
fprintf('> Importa en HyperMesh: File -> Import -> Solver Deck -> Nastran\n');
...
function write_bdf_grids_and_cbeams(fname, curves, id_start, beam)
    if nargin < 3, id_start = 1; end
    if ~isfield(beam,'E'),   beam.E = 7.0e4; end
    if ~isfield(beam,'G'),   beam.G = 2.6e4; end
    if ~isfield(beam,'NU'),  beam.NU = 0.3;  end
    if ~isfield(beam,'RHO'), beam.RHO= 0.0;  end
    if ~isfield(beam,'b'),   beam.b = 0.10;  end
    if ~isfield(beam,'h'),   beam.h = 0.10;  end
    ...
    fprintf(fid,'$ ---- Material y Propiedad ----\n');
    fprintf(fid,'MAT1,%d,%.10g,%.10g,%.10g,%.10g\n', mid, beam.E, beam.G, beam.NU, beam.RHO);
    fprintf(fid,'PBEAM,%d,%d,%.10g,%.10g,%.10g,%.10g\n', pid, mid, A, I1, I2, J);

    fprintf(fid,'$ ---- Nodos ----\n');
    nextN = id_start;
    node_ids = cell(numel(curves),1);
    for c = 1:numel(curves)
        xc = curves{c}.x(:); yc = curves{c}.y(:);
        ids = zeros(numel(xc),1);
        for k = 1:numel(xc)
            ids(k) = nextN;
            fprintf(fid,'GRID,%d,0,%.10g,%.10g,0\n', nextN, xc(k), yc(k));
            nextN = nextN + 1;
        end
        node_ids{c} = ids;
    end
    ...
    for c = 1:numel(curves)
        ...
        for k = 1:(numel(ids)-1)
            GA = ids(k); GB = ids(k+1);
            fprintf(fid,'CBEAM,%d,%d,%d,%d,0,0,1\n', eid, pid, GA, GB);
            eid = eid + 1;
        end
    end
    fclose(fid);
